...
function ContextGate({ loading, error, activeWorkspace, children }: { loading: boolean; error: Error | null; activeWorkspace: { organizationName: string; name: string; environment: string } | null; children: ReactNode }) {
  ...
  const title = loading ? "Carregando o contexto do workspace" : error ? "Não foi possível carregar o workspace" : "Selecione um workspace para continuar";
  const description = loading ? "Aguardando organização, ambiente e recursos autorizados." : error ? "O console não exibirá recursos locais enquanto o control plane não estiver disponível." : "Agentes, conexões, ferramentas e execuções pertencem a um workspace e não são exibidos fora desse contexto.";
