...
import { Activity, BarChart3, Bot, ChevronDown, GitBranch, Inbox, LayoutGrid, Menu, Plug2, Search, Settings2, X, Store } from "lucide-react";
...
export function AppShell({ children, title, action }: { children: ReactNode; title?: string; action?: ReactNode }) {
  ...
  const setHydrated = useNexo((s) => s.setHydrated); const workspaceId = useNexo((s) => s.workspaceId); const backendReady = useNexo((s) => s.backendReady); const [mobileOpen, setMobileOpen] = useState(false); const [searchOpen, setSearchOpen] = useState(false); const [query, setQuery] = useState("");
  const { workspaces, activeWorkspace, selectWorkspace, loading: contextLoading } = useWorkspaceData();
