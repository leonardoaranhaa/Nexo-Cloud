...
import { Activity, AlertCircle, BarChart3, Bot, CalendarDays, CheckCircle2, ChevronDown, GitBranch, Inbox, LayoutGrid, LoaderCircle, LogIn, Menu, Plug2, Search, Settings2, X, Store } from "lucide-react";
import { NexoWordmark } from "./brand";
import { cn } from "@/lib/utils";
import { useNexo } from "@/lib/store";
import { useWorkspaceData } from "@/lib/multitenancy/use-workspace-data";
import { useConnectionMonitor } from "@/lib/multitenancy/use-connection-monitor";
...
function ContextState({ loading, error, activeWorkspace, anonymous }: { loading: boolean; error: Error | null; activeWorkspace: { organizationName: string; name: string; environment: string } | null; anonymous: boolean }) {
  ...
  if (anonymous) return <div className="flex items-center gap-2 text-xs text-muted"><span className="size-2 rounded-full bg-accent" />Modo de exploração</div>;
...
function ContextGate({ loading, error, activeWorkspace, anonymous, allowAnonymous, children }: { loading: boolean; error: Error | null; activeWorkspace: { organizationName: string; name: string; environment: string } | null; anonymous: boolean; allowAnonymous: boolean; children: ReactNode }) {
  if (anonymous && allowAnonymous) return <>{children}</>;
  if (!loading && !error && activeWorkspace) return <>{children}</>;
  const signedOut = anonymous && !loading;
  const title = loading ? "Carregando o contexto do workspace" : signedOut ? "Explore o Nexo Cloud" : error ? "Não foi possível carregar o workspace" : "Escolha onde deseja trabalhar";
  const description = loading ? "Aguardando organização, ambiente e recursos autorizados." : signedOut ? "Navegue pela visão geral, pelo guia e pelo Marketplace. Entre ou crie uma conta quando quiser criar, conectar ou operar agentes." : error ? "O console não exibirá recursos locais enquanto o control plane não estiver disponível." : "Workspace é o projeto ou operação onde vivem seus agentes, conexões, testes e publicações.";
  return <div className="mx-auto flex min-h-[min(28rem,60vh)] max-w-xl items-center justify-center"><div className="w-full rounded-xl border border-border bg-surface p-6 text-center shadow-[var(--shadow-soft)]"><div className="mx-auto flex size-10 items-center justify-center rounded-full bg-elevated text-accent">{loading ? <LoaderCircle className="size-5 animate-spin" /> : <AlertCircle className="size-5" />}</div><h1 className="mt-4 font-display text-xl font-semibold">{title}</h1><p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-muted">{description}</p>{error && <p className="mt-4 text-xs text-warn">Tente atualizar o console ou verifique o acesso ao control plane.</p>}{signedOut && <div className="mt-5 flex flex-wrap justify-center gap-2"><Button asChild><Link to="/login"><LogIn className="size-4" />Entrar ou criar conta</Link></Button><Button variant="ghost" asChild><Link to="/">Voltar à visão geral</Link></Button></div>}</div></div>;
}

export function AppShell({ children, title, action, allowAnonymous = false }: { children: ReactNode; title?: string; action?: ReactNode; allowAnonymous?: boolean }) {
  const pathname = useRouterState({ select: (s) => s.location.pathname });
  ...
  const { workspaces, activeWorkspace, selectWorkspace, refresh, isFirstWorkspace, loading: contextLoading, error: contextError } = useWorkspaceData();
  const shellLoading = authPending || contextLoading;
  useConnectionMonitor();
  const allConnections = useNexo((s) => s.connections);
  const disconnectedConnections = useMemo(() => allConnections.filter((connection) => ["evolution", "meta", "instagram", "messenger"].includes(connection.provider) && connection.status !== "connected"), [allConnections]);
  ...
  return <div className="min-h-dvh bg-bg text-fg">
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r border-border bg-surface md:flex"><div className="border-b border-border px-5 py-5"><Link to="/" className="block"><NexoWordmark /></Link><div className="mt-3"><ContextState loading={shellLoading} error={contextError} activeWorkspace={activeWorkspace} anonymous={anonymous} /></div></div><div className="flex-1 overflow-y-auto px-2 py-5"><ServiceNav /></div><div className="border-t border-border px-4 py-4">{anonymous ? <div><div className="text-xs text-subtle">Modo de exploração</div><p className="mt-1 text-xs leading-relaxed text-muted">Veja como o Nexo funciona. Entre para criar e operar agentes.</p><Button className="mt-3 w-full" size="sm" asChild><Link to="/login"><LogIn className="size-3.5" />Entrar</Link></Button></div> : <><div className="text-xs text-subtle">Organização</div><div className="mt-1 truncate text-sm text-muted">{activeWorkspace?.organizationName ?? (contextLoading ? "Carregando…" : "Nenhuma selecionada")}</div><div className="mt-3 text-xs text-subtle">{workspaces.length > 1 ? "Projeto ou workspace" : "Workspace · ambiente"}</div><select aria-label="Selecionar workspace" value={workspaceId ?? ""} disabled={contextLoading || workspaces.length <= 1} onChange={(event) => void selectWorkspace(event.target.value)} className="mt-1 w-full rounded-md border border-border bg-bg px-2 py-1.5 text-xs text-fg"><option value="" disabled>{contextLoading ? "Carregando workspaces…" : workspaces.length === 0 ? "Nenhum workspace disponível" : workspaces.length === 1 ? "Selecionado automaticamente" : "Escolher projeto ou ambiente"}</option>{workspaces.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.environment}</option>)}</select>{workspaces.length > 1 && <p className="mt-2 text-[11px] leading-relaxed text-subtle">Agentes, conexões, testes e publicações pertencem ao contexto selecionado.</p>}{workspaces.length === 1 && <p className="mt-2 text-[11px] leading-relaxed text-subtle">Este é o único contexto disponível e foi selecionado automaticamente.</p>}</>}</div></aside>
    <div className="min-w-0 max-w-full md:pl-64"><header className="sticky top-0 z-20 border-b border-border bg-bg/90 px-4 backdrop-blur-sm md:px-8"><div className="flex min-w-0 h-14 items-center gap-3"><button type="button" className="rounded-md p-2 text-muted hover:bg-elevated md:hidden" aria-label="Abrir menu" onClick={() => setMobileOpen(true)}><Menu className="size-5" /></button><div className="shrink-0 md:hidden"><NexoWordmark compact /></div><div className="hidden min-w-0 items-center gap-2 text-xs text-muted md:flex"><span>Console</span><span className="text-subtle">/</span><span className="truncate font-medium text-fg">{title}</span></div><div className="ml-auto flex min-w-0 flex-wrap items-center justify-end gap-2"><ContextState loading={shellLoading} error={contextError} activeWorkspace={activeWorkspace} anonymous={anonymous} />{disconnectedConnections.length > 0 && !anonymous && <Link to="/connections" className="flex max-w-full items-center gap-1.5 truncate rounded-md border border-danger/40 bg-danger/10 px-2 py-1.5 text-xs text-danger" title="Ver conexões desconectadas"><AlertCircle className="size-3.5 shrink-0" /><span className="truncate">{disconnectedConnections.length === 1 ? "1 conexão desconectada" : `${disconnectedConnections.length} conexões desconectadas`}</span></Link>}<button type="button" onClick={openSearch} className="hidden h-8 items-center gap-2 rounded-md border border-border px-3 text-xs text-muted hover:bg-elevated sm:flex"><Search className="size-3.5" />Buscar serviços<span className="ml-2 rounded border border-border px-1.5 py-0.5 font-mono text-[10px]">⌘K</span></button><button type="button" onClick={openSearch} className="rounded-md p-2 text-muted hover:bg-elevated sm:hidden" aria-label="Buscar serviços"><Search className="size-4" /></button><select aria-label="Selecionar ambiente" value={activeWorkspace?.environment ?? "development"} disabled={!activeWorkspace} onChange={(event) => { const target = workspaces.find((item) => item.organizationId === activeWorkspace?.organizationId && item.environment === event.target.value); if (target) void selectWorkspace(target.id); }} className="hidden h-8 rounded-md border border-border bg-bg px-2 text-xs text-muted sm:block"> <option value="development">Development</option><option value="staging">Staging</option><option value="production">Production</option></select>{!anonymous && action}{anonymous ? <Button variant="secondary" size="sm" asChild><Link to="/login"><LogIn className="size-3.5" />Entrar</Link></Button> : <UserButton />}</div></div></header><main className="min-w-0 max-w-full overflow-x-hidden px-4 pt-6 pb-24 md:px-8 md:pb-10"><ContextGate loading={shellLoading} error={contextError} activeWorkspace={activeWorkspace} anonymous={anonymous} allowAnonymous={allowAnonymous}>{children}</ContextGate></main></div>
    {isFirstWorkspace && activeWorkspace && !contextError && <WorkspaceOnboardingDialog open workspaceId={activeWorkspace.id} initialName={activeWorkspace.name} onCompleted={() => refresh(activeWorkspace.id)} />}
