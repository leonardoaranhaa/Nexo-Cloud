...
export function useWorkspaceData() {
  const setWorkspaceSnapshot = useNexo((s) => s.setWorkspaceSnapshot); const setWorkspaceContext = useNexo((s) => s.setWorkspaceContext); const workspaceId = useNexo((s) => s.workspaceId); const [loading, setLoading] = useState(true); const [error, setError] = useState<Error | null>(null); const [contextLoaded, setContextLoaded] = useState(false); const [isFirstWorkspace, setIsFirstWorkspace] = useState(false);
  ...
  useEffect(() => { let cancelled = false; async function bootstrap() { setLoading(true); try { const context = await getWorkspaceContext(); if (cancelled) return; const persisted = useNexo.getState().workspaceId; const active = context.workspaces.find((item) => item.id === persisted) ?? context.activeWorkspace; await load(active, context.workspaces); if (!cancelled) { setIsFirstWorkspace(context.isFirstWorkspace && !active.onboardingCompleted); setContextLoaded(true); setError(null); } } catch (cause) { if (!cancelled) setError(cause instanceof Error ? cause : new Error("Falha ao carregar contexto")); } finally { if (!cancelled) setLoading(false); } } void bootstrap(); return () => { cancelled = true; }; }, [load]);
