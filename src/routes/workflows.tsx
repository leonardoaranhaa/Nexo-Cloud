...
import { AlertTriangle, GitBranch, Link2, Play, Plus, RefreshCw, Rocket, Save, Trash2 } from "lucide-react";
...
const nodeLabels: Record<WorkflowNode["type"], string> = { agent: "Agente", tool: "Ferramenta", condition: "Condição", wait: "Espera", approval: "Aprovação" };
...
function WorkflowsPage() {
  ...
  function addNode(type: WorkflowNode["type"]) {
    ...
    const defaults: JsonObject = type === "condition" ? { field: "source", equals: "manual" } : {};
  ...
  return <AppShell title="Workflows">
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(14rem,20rem)_minmax(0,1fr)]">
      ...
      <div className="space-y-4"><Card className="p-5">{selected ? <><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-display font-semibold">{selected.name}</p><p className="text-sm text-muted">{definition.nodes.length} nós · {definition.edges.length} conexões · {selected.status}</p></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="secondary" onClick={() => void save()}><Save className="size-3.5" /> Salvar</Button><Button size="sm" variant="secondary" onClick={() => void publish()} disabled={!canPublish} title={!canPublish ? "Corrija os critérios de validação" : undefined}><Rocket className="size-3.5" /> Publicar</Button><Button size="sm" onClick={() => void run()} disabled={!selected.versionId}><Play className="size-3.5" /> Executar</Button></div></div>
