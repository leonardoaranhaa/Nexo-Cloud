...
import { ArrowLeft, Check, Download, Lock, Plug, Rocket } from "lucide-react";
...
function ProductPage() {
  ...
  if (!product) return <AppShell title="Marketplace"><Card className="p-6 text-sm text-muted">Carregando produto...</Card></AppShell>;
  const manifest = product.manifest as { editableFields?: string[]; protectedComponents?: string[]; requiredConnectors?: string[]; persona?: string; welcomeMessage?: string };
  ...
  return <AppShell title="Marketplace"><Link to="/marketplace" className="mb-6 inline-flex items-center gap-2 text-sm text-muted hover:text-foreground"><ArrowLeft className="size-4" /> Voltar ao catálogo</Link>
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_22rem]"><main className="space-y-5"><Card className="p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><Badge tone="live">Nexo Cloud</Badge><h1 className="mt-3 font-display text-3xl font-semibold tracking-tight">{product.name}</h1><p className="mt-3 max-w-2xl text-sm leading-relaxed text-muted">{product.description}</p></div><Badge>v{product.versionNumber}</Badge></div><div className="mt-6 grid gap-3 sm:grid-cols-3"><div className="rounded-md bg-elevated p-3"><p className="text-xs text-muted">Categoria</p><p className="mt-1 text-sm font-medium">{product.category}</p></div><div className="rounded-md bg-elevated p-3"><p className="text-xs text-muted">Oferta</p><p className="mt-1 text-sm font-medium">{product.offerMode === "trial" ? "Trial interno" : product.offerMode}</p></div><div className="rounded-md bg-elevated p-3"><p className="text-xs text-muted">Risco</p><p className="mt-1 text-sm font-medium">{product.riskLevel}</p></div></div></Card>
      <Card className="p-6"><h2 className="font-display font-semibold">O que será instalado</h2><div className="mt-4 space-y-3 text-sm">{["Um agente draft isolado no workspace", "Manifesto e versão inicial do produto", "Entitlement interno para uso do agente", "Configuração pronta para customização"].map((item) => <p key={item} className="flex items-center gap-2"><Check className="size-4 text-success" />{item}</p>)}</div></Card>
    </main><aside className="space-y-4"><Card className="p-5"><Button className="w-full" onClick={() => void install()} disabled={installing}><Download className="size-4" /> {installing ? "Instalando..." : "Instalar no workspace"}</Button><p className="mt-3 text-center text-xs text-muted">A instalação é idempotente e começa como rascunho.</p></Card><Card className="p-5"><h2 className="font-display font-semibold">Configuração</h2><div className="mt-4 space-y-3 text-sm"><p className="flex gap-2"><Plug className="size-4 shrink-0 text-muted" /> {manifest.requiredConnectors?.length ? `Conector: ${manifest.requiredConnectors.join(", ")}` : "Sem conector obrigatório"}</p><p className="flex gap-2"><Rocket className="size-4 shrink-0 text-muted" /> Publicação manual após teste</p><p className="flex gap-2"><Lock className="size-4 shrink-0 text-muted" /> Componentes críticos protegidos</p></div></Card><Card className="p-5"><h2 className="font-display font-semibold">Campos editáveis</h2><div className="mt-3 flex flex-wrap gap-2">{(manifest.editableFields ?? []).map((field) => <Badge key={field}>{field}</Badge>)}</div></Card></aside></div>
  </AppShell>;
}
