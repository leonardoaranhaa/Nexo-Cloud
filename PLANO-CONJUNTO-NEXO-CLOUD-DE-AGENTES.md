...
**Última consolidação:** 2026-09-19 — onboarding do workspace inicial corrigido para persistir a reentrada até a conclusão; testes e build aprovados; Production ainda requer validação final de banco, login, workspace e credenciais externas.
...
**Estado: aplicação publicada permanentemente na Vercel; banco/cofre database implementados no código e Railway provisionado, mas Production ainda não configurada nem validada. AWS, cobrança automática e operação externa completa permanecem postergadas.**
...
A camada hospedada permanece bloqueada até a rotação da credencial PostgreSQL exposta na automação e a configuração explícita de `DATABASE_URL`, `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, `VITE_AUTH_ENABLED`, `NEXO_SECRETS_BACKEND` e `NEXO_SECRETS_ENCRYPTION_KEY` no ambiente Production da Vercel. Depois disso ainda será necessário validar login, vínculo owner/admin, cadastro da Evolution, healthcheck, QR e um teste externo inbound/outbound; nenhum desses passos deve ser declarado concluído antecipadamente.

## 6. Migrations, rotas e validação atual

O repositório possui migrations até `0054_account_preferences_and_secret_vault.sql`, cobrindo Marketplace, protocolo de decisão, RAG, CRM, Learning, Improvement Lab, domínio de execuções de ferramentas, blueprints persistidos, avaliações offline, disponibilidade e reserva de agenda, contratos de ferramentas comerciais, revisões de instalação, quotas diárias do runtime, auditoria correlacionável do Nexo Bot, workflows de erro, produtos próprios do Nexo, propostas governadas de tools, vínculos de workflows gerados, snapshots de avaliação, integridade multi-tenant em banco, comparação de versões, hardening de promoção, limites de segurança do WhatsApp e definições de conectores sociais da Meta.
...
| 2026-09-15 | Fatia de identidade e cofre concluída localmente: migration `0054_account_preferences_and_secret_vault.sql` adiciona preferências persistentes e vault server-side AES-256-GCM com escopo workspace/conexão, `account-server` e endpoints autenticados para perfil/preferências, e `/settings` passa a oferecer conta, sessões, preferências, governança e links operacionais. Evolution, Meta, healthcheck, Tool Gateway, runtime interno e webhooks usam o provider configurado com SQL quando aplicável; Production exige `NEXO_SECRETS_ENCRYPTION_KEY` dedicada com pelo menos 32 caracteres. Typecheck, lint, build, smoke do preview, `check:auth` contra o dev server em `localhost:8080`, migration smoke e suíte integral passaram (193/193). A configuração de Production ainda não foi concluída: a `DATABASE_URL` anteriormente exposta deve ser rotacionada no Railway antes de configurar Vercel; não declarar cofre, credencial Evolution, QR ou inbound/outbound reais validados até concluir essa etapa. |
| 2026-09-16 | Correção de autenticação Production publicada no commit `8467de8`: Better Auth agora inclui os origins exatos fornecidos pelo Vercel (`VERCEL_URL`, `VERCEL_BRANCH_URL` e `VERCEL_PROJECT_PRODUCTION_URL`) além do domínio permanente e localhost, rejeita wildcards nessa camada e possui teste dedicado. O runtime havia confirmado `Invalid origin`/403 no hostname único do deployment; gates passaram com 199 testes, typecheck, lint e build local. É necessário redeployar e validar sign-up/sign-in no alias Production antes de declarar o login operacional. |
| 2026-09-19 | Onboarding do workspace inicial corrigido: `getWorkspaceContext` e `useWorkspaceData` agora derivam `isFirstWorkspace` do estado persistido `onboarding_completed`, não apenas da criação no request. Um workspace criado mas incompleto reabre o modal após reload, sign-out/sign-in ou refresh; workspace concluído não reabre. O bootstrap existente também devolve os campos persistidos de onboarding. Adicionado teste dedicado para primeiro login, reload/reentrada, conclusão e ausência de workspace. Suíte passou com 203 testes, typecheck, lint e build. |
| 2026-09-19 | Após sincronizar commits remotos que habilitaram execução imediata do worker Evolution e seleção Anthropic, os testes integrados foram endurecidos: o webhook aceita modelo injetado apenas para fixtures e permite desabilitar o worker imediato em testes unitários; a produção mantém a execução imediata por padrão. O runtime passou a registrar o provider/modelo efetivamente usados, sem depender de variáveis de ambiente quando há modelo injetado. Suíte corrigida: 203/203, typecheck, lint e build passaram. |
